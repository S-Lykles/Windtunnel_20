...
data_3d_num_vlm = pd.read_csv(p_num/'3D data_LLT/T1-54.4 m s-LLT-x0.000mm.txt', sep=' ', header=4, skipinitialspace=True)
data_3d_num_vlm = data_3d_num_vlm.drop(0).apply(pd.to_numeric)
...
def plot_Cd_a(savefig=False):
    ...
    Cd = data_3d_exp.CD
    alpha = data_3d_exp.Alpha
    ax.plot(alpha, Cd, **style_3d_exp, label='3D experimental')
    ax.legend()
    if savefig:
...
def plot_polar(regression=False, savefig=False):
    ...
    if regression:
        file_label = '_regression'
    if savefig:
        fig.savefig('Plots/drag_polar_3d_exp_num'+file_label+'.pdf')

def plot_moment_polar(regression=False, savefig=False):
    fig = plt.figure(figsize=(8,5))
    ax = fig.add_subplot(111)
    fig.set_dpi(120)
    ax.grid(True, which='major', alpha=1, lw=0.5, ls='-')
    ...
    # --3D num--
    CL = data_3d_num.CL
    Cm_num = data_3d_num.Cm
    ax.plot(Cm_num, CL, **style_3d_num, label='3D Numerical')
    if regression:
        n_terms = 13
        p = polyfit(CL[:n_terms], Cm_num[:n_terms], [1, 0])
        r2 = r2_score(Cm_num[:n_terms], polyval(CL[:n_terms], p))
        CL_fit = np.linspace(-0.3, 1.4)
        Cm_num_fit = p[1]*CL_fit + p[0]
        fit_label = f'3D numerical regression curve'
        ax.plot(Cm_num_fit, CL_fit, ls='--', c='skyblue', label= fit_label)
        ax.annotate(f'$C_D = {p[1]:.3} \cdot C_L  {p[0]:+.3} $\n $r^2 = {r2:.4}$', (-0.20, 0.1))
    # --3D exp--
    CL = data_3d_exp.CL
    Cm_exp = data_3d_exp.Cm_p_qc
    ax.plot(Cm_exp, CL, **style_3d_exp, label='3D experimental')
    if regression and False:
        n_terms = 10
        p = polyfit(CL[:n_terms], Cm_exp[:n_terms], [1, 0])
        r2 = r2_score(Cm_exp[:n_terms], polyval(CL[:n_terms], p))
        CL_fit = np.linspace(-CL.max(), CL.max())
        Cm_exp_fit = p[2]*CL_fit**2 + p[0]
        fit_label = f'3D experimental regression curve'
        ax.plot(Cm_exp_fit, CL_fit, ls='--', c='tab:red', label= fit_label)
        ax.annotate(f'$C_D = {p[2]:.3} \cdot C_L^2  {p[0]:+.3} $\n $r^2 = {r2:.4}$', (0.025, 0.2))
    ax.legend()
    # AR = 5.345
    # print(f'e = {1/(p[2]*np.pi*AR):.2}')
    ...
    if regression:
        file_label = '_regression'
    if savefig:
        fig.savefig('Plots/moment_polar_3d_exp_num'+file_label+'.pdf')
# %%
...
# %%
plot_Cl_a(regression=True, savefig=True)
# %%
plot_moment_polar()
# %%
plot_polar(regression=True, savefig=True)
# %%
plot_polar(savefig=True)
# %%
plot_moment_polar(regression=True, savefig=True)
# %%
